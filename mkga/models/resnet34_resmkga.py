...
class ResNet34_ResMKGA(nn.Module):
    def __init__(self, num_classes=2, ablation_dict=None):
        ...
        if ablation_dict is not None:
            ...
            self.use_se_bridge = ablation_dict.get('use_se_bridge', True)
        ...
        self.bridge_conv = nn.Sequential(nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), nn.ReLU())
        if self.use_se_bridge:
            self.bridge_se = SEBlock(512)
        self.dec4 = MKGABlock(512, 256, 256, use_gate=use_gate, use_multi_kernel=use_multi_kernel, kernel_combo=kernel_combo)
        self.dec3 = MKGABlock(256, 128, 128, use_gate=use_gate, use_multi_kernel=use_multi_kernel, kernel_combo=kernel_combo)
        self.dec2 = MKGABlock(128, 64, 64, use_gate=use_gate, use_multi_kernel=use_multi_kernel, kernel_combo=kernel_combo)
        self.dec1 = MKGABlock(64, 64, 32, use_gate=use_gate, use_multi_kernel=use_multi_kernel, kernel_combo=kernel_combo)
        self.head_seg = nn.Conv2d(32, 1, 1)
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.head_pos = nn.Sequential(nn.Linear(512, 256), nn.ReLU(), nn.Linear(256, 3))
        self.head_tirads = nn.Sequential(nn.Linear(512, 256), nn.ReLU(), nn.Linear(256, num_classes))
    ...
    def forward(self, img):
        ...
        x5 = self.enc5(x4)
        if self.use_se_bridge:
            bridge = x5 + self.bridge_se(self.bridge_conv(x5))
        else:
            bridge = x5 + self.bridge_conv(x5)
        ...
        feat = self.gap(bridge).flatten(1)
        return mask, self.head_pos(feat), self.head_tirads(feat)
